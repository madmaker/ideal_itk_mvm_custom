...
/*
 * Считывается item_id M9_KD. С конца строки до первого пробела считывается код документа.
 *
 * Если кода документа нет, то ищем в TC M9_CompanyPart с атрибутом item_id = item_id m9_KD.
 * Если такого M9_CompanyPart нет, тогда m9_DocumentType равен пусто.
 * Если есть и у M9_CompanyPart атрибут m9_TypeOfPart равен Part, тогда m9_DocumentType = "". Иначе m9_DocumentType = Спецификация.
 * Если есть и у M9_CompanyPart атрибут m9_TypeOfPart равен пусто, тогда m9_DocumentType = Тип не определен.
 *
 * Если код документа есть, то ищем его значение в значении preferences.
 * Если в preferences код документа есть или есть код документа, но без числового значения (например, в preferences есть СБ, но нет СБ1), тогда присваивать m9_DocumentType значение из preferences.
 * Если кода документа в preferences нет, тогда m9_DocumentType=Тип не определен.
 */

int m9_get_document_type_value(METHOD_message_t *mess, va_list args) {
	try {
		va_list vargs;
		va_copy(vargs, args);
		...
		if (posOfSpace != string::npos) {
			...
			char **values = NULL;
			PREF_initialize();
			erc = PREF_ask_char_values("M9_DocumentTypes", &n_values, &values);
			if (debug)
				printf("\t n_values: %d \n", n_values);
			...
			if (query_t != NULL_TAG) {
				...
				if (resultCount != 0) {
					...
					if (found_kit != NULL_TAG) {
						erc = AOM_ask_value_string(found_kit, "m9_TypeOfPart",
								&typeOfPart);
